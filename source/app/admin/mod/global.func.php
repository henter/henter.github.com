...
function atpl($template = 'index',$mod='dc')
{
	...
	$compiledtplfile = TPL_CACHEPATH_ADMIN.$templatemod.'.tpl.php';
       ...
       if(!file_exists($tplfile)) showmessage("后台模板文件 $mod $template  不存在！");
	if(TPL_REFRESH && (!file_exists($compiledtplfile) || @filemtime($tplfile) > @filemtime($compiledtplfile)))
	{
		template_compile_admin($template,$mod);
	}
...
function phptpl($template = 'index',$mod='dc')
{
       $modurl = ($mod=='dc') ? ADMIN_PATH.'/' : $mod.'/admin/';
	$tplfile = DC_ROOT.$modurl."templates/$template.php";
	return $tplfile;
}
