function open_linkage(id,name,container,linkageid) {
	...
	content += '</div></div>';
	window.top.art.dialog({title:name,id:'edit_'+id,content:content,width:'422',height:'200',style:'none_icon ui_content_m'});
}
...
function get_child(nodeid,nodename,parentid) {
	...
	$.getJSON(url+'&callback=?',function(data){
		if(data) {
			...
			window.top.art.dialog({id:'edit_'+returnid}).close();
		}
	})
}
